...
class SharedActions {
    ...
    protected:
		...
		float goal_position_linear(ProximitySector& data, float w);
		...
		/*!
		 * Protected method to check if the element projects inside device's
		 * radius.
		 * @param distance	Distance of the element
		 * @param angle		Angle of the element
		 * @param radius	Radius of the device
		 * @return			True if the element projection is inside the
		 *					device's radius. False, otherwise.
		 */
		virtual bool is_projection_inside(float distance, float angle, float radius);

		virtual bool is_within_goal_area(float area_radius);
	...
	private:
		...
		bool on_request_set_goal(std_srvs::Empty::Request& req,
									 std_srvs::Empty::Response& res);

		geometry_msgs::PolygonStamped create_goal_area(float radius);

		void on_receive_odometry(const nav_msgs::Odometry& data);
